...
pub fn find_shortest_path(
    ...
    while !open_set.is_empty() {
        ...
        for neighbour in get_neighbours(map, current.pos) {
            if is_wall(map, neighbour) || blacklist.contains(&neighbour) {
                continue;
            }
            ...
            let tentative_g_score: f64 =
                *g_scores.get(&current.pos).unwrap() + get_heuristic_cost(current.pos, neighbour);
...
fn is_wall(map: &Map, pos: Position) -> bool {
    map[(pos.x, pos.y)]
}
...
fn get_neighbours(map: &Map, node: Position) -> Vec<Position> {
...
pub fn draw_map(map: &Map) {
    ...
    for y in 0..map.height {
        for x in 0..map.width {
            if is_wall(map, Position::new(x, y)) {
                print!("#");
            } else {
                print!(".");
            }
...
#[allow(dead_code)]
pub fn draw_map_with_path(map: &Map, path: &[Position]) {
